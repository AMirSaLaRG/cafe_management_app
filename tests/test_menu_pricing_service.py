    ...
    def test_error_handling_comprehensive(self, in_memory_db):
        ...
        class TestDirectCostUpdates:
            ...
            def test_add_recipe_updates_menu_price(self, in_memory_db):
                ...
                # Check that price was recalculated
                updated_record = in_memory_db.get_estimatedmenupricerecord(
                    menu_id=coffee.id, row_num=1
                )[0]

                # Cost should have increased by 10g * $0.02 = $0.20
                assert updated_record.direct_cost == pytest.approx(
                    initial_record.direct_cost + 0.20, 0.01
                )

            def test_change_recipe_updates_price(self, in_memory_db):
                ...
                # New cost: 20g * $0.05 = $1.00
                assert updated_record.direct_cost == pytest.approx(1.00, 0.01)
        ...
        class TestMenuManagement:
            ...
            def test_add_menu_item_with_pricing(self, in_memory_db):
                ...
                assert menu_item.current_price == 6.50
        ...
        class TestRealWorldScenariosExtended:
            """Extended real-world scenario tests"""
            ...
            def test_supply_chain_crisis_response(self, in_memory_db):
                ...
                service.change_recipe_of_menu_item(
                    menu_id=vanilla_latte.id,
                    inventory_id=vanilla.id,
                    amount=10.0,  # Reduce from 15ml to 10ml
                    note="Reduced vanilla due to supply issues"
                )
            ...
            def test_new_product_launch(self, in_memory_db):
                ...
                matcha_latte = service.add_menu_item(
                    ...
                    value_added_tax=0.09,
                    current_price=7.50,  # Initial price estimate
    ...
    class TestIntegrationScenarios:
        ...
        def test_complete_pricing_workflow_with_manual_approval(self, in_memory_db):
            ...
            service.add_recipe_of_menu_item(
                menu_id=latte.id,
                inventory_id=coffee.id,
                amount=20.0,  # 20g coffee
                writer="Head Barista",
                note="Double shot espresso"
            )

            service.add_recipe_of_menu_item(
                menu_id=latte.id,
                inventory_id=milk.id,
                amount=300.0,  # 300ml milk
                writer="Head Barista",
                note="Steamed milk"
            )
